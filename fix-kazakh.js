// Script to fix Kazakh translations based on verification report
const fs = require('fs');
const path = require('path');

// Read verification report
const verificationPath = path.join(__dirname, 'reviews', 'kk_verification.json');
...
console.log('Starting Kazakh translation fixes...\n');
...
verification.results.forEach((result, index) => {
  ...
  if (result.status === 'verified') {
    ...
    if (phraseStart !== -1) {
      ...
      // Check if kazakh field already exists
      if (phraseBlock.includes('kazakh:')) {
        // Replace existing kazakh field
        const kazakhPattern = /kazakh:\s*"[^"]*"/;
        const newPhraseBlock = phraseBlock.replace(kazakhPattern, `kazakh: "${cleanedTranslation}"`);
        phrasesContent = phrasesContent.substring(0, start) + newPhraseBlock + phrasesContent.substring(end + 1);
      } else {
        // Add kazakh field before the closing brace
        // Find the last comma
