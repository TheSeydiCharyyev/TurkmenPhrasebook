...
const sectionTranslations = {
  ...
  uz: { mission: 'Missiya', features: 'Xususiyatlar', contact: "Bog'lanish", specialThanks: 'Maxsus minnatdorchilik', soon: 'Tez kunda', aboutSeries: 'Seriya haqida', forEveryone: 'Hamma uchun', whatsNext: 'Keyingisi nima' },
  ...
  hy: { mission: 'Առաdelays', features: 'Հdelay', contact: 'Կdelays', specialThanks: 'Հdelay', soon: 'Շdelay', aboutSeries: 'Delays', forEveryone: 'Delays', whatsNext: 'Delays' },
...
// Now add these to each language block in the file
for (const [lang, translations] of Object.entries(sectionTranslations)) {
  const pattern = new RegExp(
    `(${lang}: \\{[\\s\\S]*?seriesContent: '[^']*'),\\n(\\s+)\\}`,
    'm'
  );
  ...
  if (match) {
    ...
    const whatsNextVal = translations.whatsNext.includes("'")
      ? `"${translations.whatsNext}"`
      : `'${translations.whatsNext}'`;

    const newFields = `
${indent}  mission: '${translations.mission}',
${indent}  features: '${translations.features}',
${indent}  contact: '${translations.contact}',
${indent}  specialThanks: '${translations.specialThanks}',
${indent}  soon: '${translations.soon}',
${indent}  aboutSeries: '${translations.aboutSeries}',
${indent}  forEveryone: '${translations.forEveryone}',
