// Script to fix Azerbaijani translations based on verification report
const fs = require('fs');
const path = require('path');

// Read verification report
const verificationPath = path.join(__dirname, 'reviews', 'az_verification.json');
...
console.log('Starting Azerbaijani translation fixes...\n');
...
verification.results.forEach((result, index) => {
  ...
  if (result.status === 'verified') {
    ...
    if (phraseStart !== -1) {
      ...
      // Check if azerbaijani field already exists
      if (phraseBlock.includes('azerbaijani:')) {
        // Replace existing azerbaijani field
        const azerbaijaniPattern = /azerbaijani:\s*"[^"]*"/;
        const newPhraseBlock = phraseBlock.replace(azerbaijaniPattern, `azerbaijani: "${cleanedTranslation}"`);
        phrasesContent = phrasesContent.substring(0, start) + newPhraseBlock + phrasesContent.substring(end + 1);
      } else {
        // Add azerbaijani field before the closing brace
        // Find the last comma
        ...
        if (lastCommaPos !== -1) {
          ...
          phrasesContent = phrasesContent.substring(0, insertPos) +
                          `\n    azerbaijani: "${cleanedTranslation}",` +
